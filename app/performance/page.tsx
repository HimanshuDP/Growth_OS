...
const SentimentWidget = dynamic(() => import('@/components/ml/SentimentWidget'), { ssr: false });
...
export default function PerformancePage() {
  ...
  useEffect(() => {
    ...
    if (!rawProfile) { router.push('/onboard'); return; }
  ...
  return (
    <div className="min-h-screen bg-[#0F172A] text-slate-200 pb-24 font-sans">
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 lg:col-span-2 shadow-xl">
            ...
            <div className="h-72 w-full">
              <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1} key={data ? 'audience-ready' : 'audience-loading'}>
                <AreaChart data={metrics} margin={{ top: 5, right: 0, left: -20, bottom: 0 }}>
                  ...
                  <Area yAxisId="left" type="monotone" dataKey="impressions" name="Impressions" stroke="#FF6B35" strokeWidth={2} fillOpacity={1} fill="url(#colorImp)" />
                  <Area yAxisId="left" type="monotone" dataKey="reach" name="Reach" stroke="#00C9A7" strokeWidth={2} fillOpacity={1} fill="url(#colorReach)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>

          <div className="bg-gradient-to-br from-orange-950/30 to-slate-900 border border-brand-orange/20 rounded-2xl p-6 shadow-xl flex flex-col">
            <div className="flex items-center gap-2 text-brand-orange font-bold mb-6">
            ...
            <div className="space-y-5 flex-1 overflow-y-auto pr-2">
              ...
              <div className="bg-brand-orange/10 border border-brand-orange/20 rounded-lg p-4 mt-4">
                <h4 className="flex items-center gap-2 text-sm font-bold text-brand-orange mb-2"><Target className="w-4 h-4"/> Next Action</h4>
                <p className="text-xs text-slate-200">Reallocate 20% of your Facebook static image budget into Instagram Reels. Expected ROAS increase: <strong>+0.4x</strong> over next 7 days.</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl">
            ...
            <div className="h-60 w-full">
              <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1} key={data ? 'bar-ready' : 'bar-loading'}>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl">
            ...
            <div className="h-60 w-full">
              <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1} key={data ? 'eng-ready' : 'eng-loading'}>
        ...
        {/* ═══ ML MODULE 1: BRAND SENTIMENT MONITOR ═══ */}
        <div>
          <div className="flex items-center gap-3 mb-4">
            <div className="h-px flex-1 bg-slate-800" />
            <h2 className="text-base font-black text-white uppercase tracking-widest px-2">Brand Sentiment Monitor</h2>
            <div className="h-px flex-1 bg-slate-800" />
          </div>
          <p className="text-sm text-slate-500 mb-6 text-center">
            Real-time sentiment analysis powered by DistilBERT — runs entirely in your browser
          </p>
